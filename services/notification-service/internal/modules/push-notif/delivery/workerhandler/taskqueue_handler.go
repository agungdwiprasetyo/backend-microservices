...
func NewTaskQueueHandler(uc usecase.PushNotifUsecase, validator interfaces.Validator) *TaskQueueHandler {
...
func (h *TaskQueueHandler) MountHandlers(group *types.WorkerHandlerGroup) {
	group.Add("push-notif-task-one", h.taskOne)
	group.Add("push-notif-task-two", h.taskTwo)
}

func (h *TaskQueueHandler) taskOne(ctx context.Context, message []byte) error {
	trace := tracer.StartTrace(ctx, "PushNotifDeliveryTaskQueue:Hello")
	defer trace.Finish()
	ctx = trace.Context()
