...
func NewTaskQueueHandler(uc usecase.AuthUsecase, validator interfaces.Validator) *TaskQueueHandler {
...
func (h *TaskQueueHandler) MountHandlers(group *types.WorkerHandlerGroup) {
	group.Add("auth-task-one", h.taskOne)
	group.Add("auth-task-two", h.taskTwo)
}

func (h *TaskQueueHandler) taskOne(ctx context.Context, message []byte) error {
	trace := tracer.StartTrace(ctx, "AuthDeliveryTaskQueue:Hello")
	defer trace.Finish()
	ctx = trace.Context()
